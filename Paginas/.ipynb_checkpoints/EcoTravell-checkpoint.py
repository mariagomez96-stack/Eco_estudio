...
# ------------------------------------------------- COMPARADOR ELÉCTRICO ---------------------------------------------------------------
    ...
    if modelo_hibrido and modelo_electrico_puro:
        ...
        if not st.session_state.grafico_visible:
            ...
            funcion_comparacion_seleccion_usario_modelo(2, ["Pure Electrical Consumption", "Hybrid Consumption"], seleccionElectrico= str(modelo_electrico_puro),seleccionGasoi=None,seleccionGasolina=None ,seleccionHibrido=str(modelo_hibrido))
        with columna_1:
...
# ------------------------------------------------ Gasoline -----------------------------------------------------------------------------------------------------
    ...
    if modelo_gasolina and modelo_gasoleo:
        ...
        if not st.session_state.grafico2_visible:
            ...
            #st.write(modelo_hibrido)
            #st.write(modelo_electrico_puro)
            funcion_comparacion_seleccion_usario_modelo(2, ["Diesel Consumption", "Gasoline Consumption"], seleccionElectrico= None,seleccionGasoi=str(modelo_gasoleo),seleccionGasolina=str(modelo_gasolina) ,seleccionHibrido=None)
    else:
        with columna_2:
            boton_modelos_g = st.button(key= "o_2", label="Compare Gasoline vs Diesel")
        if boton_modelos_g:
    ...
    if modelo_electrico_puro and modelo_hibrido and modelo_gasolina and modelo_gasoleo:
        ...
        if boton_comparar:
            funcion_comparacion_seleccion_usario_modelo(4, ["Pure Electrical Consumption","Hybrid Consumption","Diesel Consumption", "Gasoline Consumption"], seleccionElectrico= str(modelo_electrico_puro),seleccionGasoi=str(modelo_gasoleo),seleccionGasolina=str(modelo_gasolina) ,seleccionHibrido=str(modelo_hibrido))
...
# ----------------------------------------------------- CALCULADOR VIAJES ---------------------------------------------------------
        if st.session_state.indices_puntos_mapa:
            ...
            with columna_2:
                input_precio_gasolina = st.number_input(label= "Current gasoline price:", step = 0.01)
                input_precio_gasoleo = st.number_input("Current diesel price:", step=0.01)
            ...
            if st.session_state.seleccion_cargador not in st.session_state.indices_puntos_mapa:
                ...
                if input_km_usuario != None and input_km_usuario > 0 and input_precio_gasoleo > 0.00 or input_precio_gasolina > 0.00:
                    ...
                    if df_electrico is not None and df_hibrido is not None and df_gasolina is not None and df_gasoleo is not None:
                        ...
                        st.write("Select a charging point where the price per kW/h is not unknown")       
                else:
